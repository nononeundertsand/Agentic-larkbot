...
process.env.LLM_MODELS = JSON.stringify([
  ...
  { id: 'text-only', temperature: 0.2, vision: false },
]);
...
test('vision 任务不会回落到不支持图片的当前默认模型', () => {
  const oldRoute = process.env.LLM_ROUTE_VISION;
  process.env.LLM_ROUTE_VISION = '';
  try {
    assert.equal(setRuntimeDefaultModel('text-only').ok, true);
    assert.equal(resolveModelChain({ task: 'vision' })[0], 'gpt-4o-mini');
  } finally {
    process.env.LLM_ROUTE_VISION = oldRoute;
    setRuntimeDefaultModel('gpt-4o-mini');
  }
});
...
test('listModelIds 覆盖默认模型与档案', () => {
  ...
  assert.ok(ids.includes('text-only'));
});
