...
基于 `lark-cli` 的 **WebSocket 长连接**接收事件，**无需公网服务器、无需内网穿透、无需回调地址**。群聊/私聊图片会按配置下载到临时目录后走多模态模型生成文本描述，再进入 Agent 上下文。

## 架构总览

```
飞书消息 / 卡片点击 →(lark-cli event consume, WebSocket)→ bot.mjs
...
| `run_python_code` | 在无网络、无 workspace 挂载的 Docker Python 沙箱运行短代码并返回 stdout/stderr | Docker | 只读宿主 |
| `run_shell_command` | 受限本地命令执行：查文件、搜代码、git 只读、显式允许的项目检查 | 沙箱 | 高风险需确认 |
...
| 机制 | 说明 |
|------|------|
| **专属主人** | 主人优先由 `.env` 的 `OWNER_OPEN_ID` / `OWNER_NAME` 配置；`OWNER_OPEN_ID` 为空时默认从 `lark-cli` 当前 user 登录态自动发现。身份锚在事件 `sender_id`，用户无法伪造。 |
...
> **确认卡片**：写操作确认卡片依赖飞书开发者后台启用 `card.action.trigger` 事件。未启用时，bot 仍会发送卡片和保留确认码兜底文本，但点击按钮不会触发执行。可用 `APPROVAL_CARD_ENABLED=off` 回退到纯文本确认。
...
- **无需加工具**：长尾飞书操作 AI 可通过元工具读取 `lark-cli skills` 内置最新版文档并运行 `lark-cli` 自动完成；本地 `feishu-skill` 只作为离线回退，目录默认不入 git。
